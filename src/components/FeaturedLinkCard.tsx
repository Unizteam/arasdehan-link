...
type FeaturedLinkCardProps = {
  ...
  badge?: string;
  accent?: AccentId;
  /** When false, renders as non-interactive content with no CTA or link wrapper. */
  interactive?: boolean;
};
...
export function FeaturedLinkCard({
  ...
  const enabled = interactive && hasLink(href);

  const articleClass = cn(
    "overflow-hidden rounded-[16px] bg-surface ring-1 ring-line shadow-[0_14px_34px_-22px_rgba(0,0,0,0.85)]",
    interactive && "transition duration-300 group-hover:bg-surface-2",
  );
    ...
    <article className={articleClass}>
      <div className="flex min-h-[136px] flex-row sm:min-h-[172px] md:min-h-[200px]">
        ...
        <div className="flex min-w-0 flex-1 flex-col justify-center px-3.5 py-3.5 sm:px-5 sm:py-4">
          ...
          <h3
            className={cn(
              "text-[1.05rem] leading-[1.55] font-semibold text-foreground sm:text-[1.32rem] sm:leading-[1.5]",
              title ? "mt-1.5" : "",
            )}
          >
            {message}
          ...
          {description ? (
            <p
              className={cn(
                "mt-1.5 leading-[1.75] text-muted",
                interactive
          ...
          {meta ? (
            <p className="mt-2 text-[12px] leading-[1.75] text-foreground/55 sm:text-[13px]">
              {meta}
            </p>
          ) : null}
